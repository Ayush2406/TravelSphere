...
Configure the backend environment variables according to the project's environment configuration.

Start the FastAPI development server using the command configured by the project, for example:
...
Configure the frontend `.env` file with the backend API base URL.
...
The exact commands may differ depending on the final project configuration.
